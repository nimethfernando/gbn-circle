...
export default function ThreePrinciples({ data }: ThreePrinciplesProps = {}) {
  const heading = data?.heading || "Three Principles. One Business Community.";
  const quote = data?.quote || "Your network can become your next opportunity.";
  ...
  const principles =
    data?.principles && data.principles.length > 0
      ? data.principles.map((item, idx) => ({
          title: item.title,
          desc: item.desc,
          color: defaultPrinciples[idx % defaultPrinciples.length].color,
        }))
      : defaultPrinciples;
